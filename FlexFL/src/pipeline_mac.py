...
# single-process, CPU/MPS-friendly init
if dist.is_available() and not dist.is_initialized():
    # env:// works fine for a singleton too
...
ckpt_dir: str = "Meta-Llama-3-8B-Instruct-GGUF/Meta-Llama-3-8B-Instruct.Q4_K_M.gguf"
...
# initialize llama.cpp model (GGUF)
generator = Llama(
    model_path=ckpt_dir,
    n_ctx=max_seq_len,
    n_threads=4,       # adjust for your CPU cores
    n_gpu_layers=35,   # tune for your VRAM; 0 disables GPU
    seed=seed,
)

# 2. Inference
def query(instruction: str):
    results = generator.create_chat_completion(
        messages=[{"role": "user", "content": instruction}],
        max_tokens=max_gen_len or 512,
        temperature=temperature,
        top_p=top_p,
    )
    content = results["choices"][0]["message"]["content"].strip()
